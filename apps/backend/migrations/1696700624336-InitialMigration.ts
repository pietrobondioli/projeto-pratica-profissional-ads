...
export class InitialMigration1696700624336 implements MigrationInterface {
  name = 'InitialMigration1696700624336';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE \`reservation\` (\`id\` varchar(36) NOT NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`createdBy\` varchar(255) NOT NULL DEFAULT 'system', \`updatedBy\` varchar(255) NOT NULL DEFAULT 'system', \`startDate\` datetime NOT NULL, \`endDate\` datetime NOT NULL, \`totalPrice\` int NOT NULL, \`paymentStatus\` enum ('Pending', 'Paid', 'Failed') NOT NULL, \`equipmentId\` varchar(36) NULL, \`renterId\` varchar(36) NULL, \`renteeId\` varchar(36) NULL, \`paymentId\` varchar(36) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`equipment\` (\`id\` varchar(36) NOT NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`createdBy\` varchar(255) NOT NULL DEFAULT 'system', \`updatedBy\` varchar(255) NOT NULL DEFAULT 'system', \`title\` varchar(255) NOT NULL, \`description\` varchar(255) NOT NULL, \`pricePerDay\` int NOT NULL, \`availabilityStatus\` tinyint NOT NULL DEFAULT 1, \`photoId\` varchar(36) NULL, \`ownerId\` varchar(36) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TABLE \`change_email_token\` (\`id\` varchar(36) NOT NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`createdBy\` varchar(255) NOT NULL DEFAULT 'system', \`updatedBy\` varchar(255) NOT NULL DEFAULT 'system', \`oldEmail\` varchar(255) NOT NULL, \`newEmail\` varchar(255) NOT NULL, \`token\` varchar(255) NOT NULL, \`expiresAt\` datetime NOT NULL, \`consumedAt\` datetime NULL, \`invalidatedAt\` datetime NULL, \`consumerIp\` varchar(255) NULL, \`userId\` varchar(36) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TABLE \`user\` (\`id\` varchar(36) NOT NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`createdBy\` varchar(255) NOT NULL DEFAULT 'system', \`updatedBy\` varchar(255) NOT NULL DEFAULT 'system', \`email\` varchar(255) NOT NULL, \`passwordHash\` varchar(255) NOT NULL, \`confirmedEmail\` tinyint NOT NULL DEFAULT 0, \`userProfileId\` varchar(36) NULL, UNIQUE INDEX \`REL_2ffc8d3455097079866bfca4d4\` (\`userProfileId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE \`reservation\` ADD CONSTRAINT \`FK_27531e0283dafb922cc29b1710d\` FOREIGN KEY (\`renteeId\`) REFERENCES \`user\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`reservation\` DROP FOREIGN KEY \`FK_27531e0283dafb922cc29b1710d\``,
    );
    await queryRunner.query(
